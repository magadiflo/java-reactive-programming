...
Imaginemos que tenemos un flujo de pedidos a través del cual seguimos recibiendo estos eventos de órdenes de compra
(`PurchaseOrder`) como cada 200 milisegundos o cada 500 milisegundos, el intervalo que queramos.

El objeto que se estará emitiendo será el siguiente:
...
Entonces, nuestro flujo sigue emitiendo órdenes de compra con muchas categorías, de las cuales a la empresa le interesa
dos categorías particulares: `Kids` y `Automotive`, así que las demás categorías las ignoraremos.

Tenemos algunas reglas de negocio para procesar caga grupo de categoría:

- Si vemos las órdenes de automóviles, tenemos que actualizar el precio agregando S/ 100.
- Si vemos las órdenes de kids, hay una promoción en marcha que es añadir 1 pedido gratis por cada órden de kids. Eso
  significa que al precio del pedido gratis podríamos colocarle como 0.
