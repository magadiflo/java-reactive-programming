...
## GroupBy - Tarea asignada

### 🎯 Enunciado

Simulamos un flujo continuo de órdenes de compra (`PurchaseOrder`) que llegan cada 200 milisegundos. Estas órdenes
contienen un producto, una categoría y un precio aleatorio generado usando la librería Faker. Sin embargo, a nuestra
empresa solo le interesan dos categorías específicas:

- Kids
- Automotive

Las demás categorías deben ser ignoradas. Además, el objeto que se estará emitiendo será el siguiente:

````java
record PurchaseOrder(String item, String category, Integer price) {
}
//faker tiene commerce() para crear este objeto con valores aleatorios:
//item: productName
//category: department
//price: aleatorio
````
...
Cada categoría tiene un procesamiento especial:

- 🧸 `Kids`: Cada orden de esta categoría tiene una promoción activa. Por cada orden, se agrega automáticamente una
  copia con precio 0 (orden gratuita).
- 🚗 `Automotive`: Cada orden debe tener un recargo de S/ 100 en su precio.

![05.png](assets/section-10/05.png)

## GroupBy - Solución a la tarea asignada

Creamos nuestro modelo de datos `PurchaseOrder`. Es una clase record que representa una orden de compra con tres
atributos. Además, hemos definido el método `create()` para generar una orden aleatoria usando `Faker`.
...
Como segundo paso hemos creado una clase de servicio `OrderProcessingService` que englobará la lógica de negocio. Esta
clase contiene:

- Un mapa `PROCESSOR_MAP` que asocia una categoría a su lógica de transformación (`UnaryOperator<Flux<PurchaseOrder>>`).
- Un método `canProcess()` que filtra las órdenes que se pueden procesar.
- Métodos individuales para procesar órdenes de `Kids` y `Automotive`.
...
Finalmente, en la clase principal orquestamos todo el flujo reactivo. Donde:

- `orderStream()` emite una orden cada 200 ms.
- Se filtran solo las categorías de interés.
- Se agrupan las órdenes por categoría usando `groupBy`.
- Se aplica el procesamiento correspondiente usando `transform`.
...
- Si llega una orden de `Kids`, se emite dos veces: la original y una versión gratuita.
- Si llega una orden de `Automotive`, se emite una versión con precio incrementado en S/ 100.
- Todas las demás categorías son descartadas silenciosamente.

````bash
11:29:27.933 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Ergonomic Granite Hat, category=Kids, price=45]
11:29:27.961 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Ergonomic Granite Hat-FREE, category=Kids, price=0]
11:29:31.730 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Practical Linen Clock, category=Kids, price=49]
11:29:31.730 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Practical Linen Clock-FREE, category=Kids, price=0]
11:29:34.723 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Awesome Steel Knife, category=Automotive, price=183]
11:29:38.725 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Synergistic Linen Bag, category=Automotive, price=133]
11:29:39.728 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Fantastic Bronze Plate, category=Automotive, price=173]
11:29:39.936 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Mediocre Rubber Wallet, category=Automotive, price=184]
11:29:53.333 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Practical Granite Bag, category=Automotive, price=149]
11:30:03.928 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Ergonomic Plastic Computer, category=Kids, price=49]
11:30:03.928 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Ergonomic Plastic Computer-FREE, category=Kids, price=0]
11:30:04.135 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Incredible Wool Shoes, category=Kids, price=86]
11:30:04.135 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Incredible Wool Shoes-FREE, category=Kids, price=0]
11:30:04.327 INFO  [     parallel-1] d.m.a.common.DefaultSubscriber :  recibido: PurchaseOrder[item=Practical Aluminum Bottle, category=Automotive, price=142]
...
````
