...
Para indicarle al productor que el consumidor tiene una capacidad limitada y evitar que produzca más de lo que puede
manejar, `Reactor` ofrece el operador `.limitRate(...)`. Por ejemplo, `.limitRate(5)` indica que el productor solo debe
solicitar (y, por lo tanto, generar) hasta 5 elementos a la vez.
