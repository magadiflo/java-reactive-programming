...
En la sección anterior, vimos cómo crear un flujo a partir de estructuras ya existentes, como listas o arreglos.
Eso funciona bien cuando los datos están disponibles de antemano, pero ¿qué pasa si queremos
`emitir datos bajo demanda`, mientras nuestro código se ejecuta?

En ocasiones necesitamos producir valores de forma `imperativa`, por ejemplo cuando queremos generar elementos
dinámicamente, o integrar un flujo con `APIs basadas en callbacks o eventos externos`. Para este tipo de escenarios,
`Reactor` nos ofrece dos operadores clave: `Flux.create()` y `Flux.generate()`.

## Flux.create() – Emisión programática e imperativa de elementos

`Flux.create()` permite construir un `Flux` de manera `imperativa`, es decir, controlando explícitamente qué elementos
se emiten, cuándo se emiten y cuándo el flujo termina. Esta técnica resulta ideal para integrar `código tradicional`
o `APIs que producen datos de forma asíncrona`, como listeners, colas o fuentes de eventos.

El método recibe un `FluxSink`, que actúa como un canal para enviar elementos al flujo reactivo.

### 🧩 Ejemplo: Emisión manual con condición de parada
...
public class Lec01FluxCreate {
    public static void main(String[] args) {
        Flux.create(fluxSink -> {
            ...
            fluxSink.complete();                            // finaliza el flujo manualmente
...
- `Flux.create(...)`. Crea un `Flux` manualmente usando un `FluxSink`. El desarrollador decide cuándo y qué emitir.
- `FluxSink<T>`. Es una interfaz que actúa como puente entre el mundo imperativo y el mundo reactivo. A través de este
  objeto podemos:
    - `next(T value)` → emitir un nuevo valor al flujo,
    - `error(Throwable e)` → emitir un error y terminar el flujo,
    - `complete()` → marcar el flujo como completado.
- `fluxSink.complete()`. Es importante llamar a este método al final de la emisión para indicar que no se producirán
  más elementos.

💡 Nota:
> El flujo anterior es finito porque el código dentro del lambda define la condición de parada (`Peru`).
> Si no se invoca `complete()`, el flujo quedaría abierto y nunca se completaría.
...
### ⚠️️ Demostrando algo que `No es Thread Safe`
...
### 🛡️ Demostrando lo que `es Thread Safe`
...
## 🧠 Concurrencia y seguridad de Hilos con `Flux.create()` y `FluxSink`

En esta lección exploramos un aspecto fundamental de `Flux.create(...)`:
su `capacidad para manejar emisiones concurrentes de forma segura` gracias a su diseño `thread-safe`.

A diferencia de otros creadores (como `Flux.generate()`), `Flux.create()` permite que
`múltiples hilos emitan elementos simultáneamente`, sin riesgo de corrupción de datos ni condiciones de carrera.
Esto lo convierte en una opción ideal cuando trabajamos con `APIs o procesos que producen datos desde diferentes hilos`
(por ejemplo, listeners, sockets, colas, etc.).

### ⚙️ Características clave de `FluxSink`

- 🧩 `Cada suscriptor obtiene su propio flujo independiente`. Cada vez que llamas a `.subscribe()`, `Reactor`
  crea una nueva instancia del flujo y vuelve a ejecutar el bloque pasado a `Flux.create(...)`. Esto significa que los
  suscriptores no comparten el mismo `FluxSink` ni el mismo estado interno. En cada suscripción se dispara nuevamente
  toda la lógica dentro de `Flux.create(...)`: creación de hilos, emisión de datos, finalización, etc.
- 🧵 `FluxSink` es `seguro para subprocesos` (`thread-safe`).
- 🚦 No es necesario preocuparse por la demanda del suscriptor (`backpressure`), ya que `Reactor` la gestiona
  internamente.
- ✅ Garantiza que todos los elementos emitidos se entreguen `de forma segura y consistente` al suscriptor.

### 💡 Ejemplo: Emisiones concurrentes con FluxSink

A continuación, un ejemplo donde `tres hilos` generan datos aleatorios y los emiten de forma segura a través del
mismo `FluxSink`:
...
- Se crean `3 hilos`, cada uno emitiendo 5 valores (`0–4`) a través del mismo `FluxSink`.
- Los valores se reciben en el suscriptor sin errores, duplicados ni conflictos de concurrencia.
- El orden de llegada puede variar (ya que los hilos se ejecutan en paralelo), pero todas las emisiones se procesan de
  forma segura.

En el resultado vemos que cada hilo emite sus propios valores del `0, 1, 2, 3 y 4`, y el suscriptor los recibe en
orden no determinístico, según la planificación del sistema operativo.

👉 En otras palabras: los valores pueden aparecer mezclados, pero todos provienen de emisiones válidas y seguras
de cada hilo independiente.

````bash
17:17:59.261 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-0, valor -> 0
17:17:59.266 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-2, valor -> 0
17:17:59.266 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-1, valor -> 0
17:17:59.266 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-2, valor -> 1
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-1, valor -> 1
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-2, valor -> 2
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-2, valor -> 3
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-1, valor -> 2
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-2, valor -> 4
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-1, valor -> 3
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-1, valor -> 4
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-0, valor -> 1
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-0, valor -> 2
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-0, valor -> 3
17:17:59.267 INFO  [       Thread-0] .a.s.Lec04FluxSinkThreadSafety : Recibido: Hilo -> Thread-0, valor -> 4
````

### 🧩 Conclusiones

- ✅ `Flux.create()` es `thread-safe`, lo que permite que múltiples productores (`hilos`) emitan en paralelo.
- ⚠️ Si se usara `Flux.generate()` en el mismo escenario, se producirían errores de concurrencia porque no es seguro
  para hilos.
- 💬 En escenarios del mundo real, esta característica permite integrar `Flux.create()` con APIs de eventos asíncronos
  o sistemas que producen datos desde varios contextos de ejecución.
...
## ⚙️ Flux.generate() – Generación síncrona y controlada de datos

`Flux.generate()` crea un flujo síncrono y secuencial en `Project Reactor`. Está pensado para escenarios donde
queremos `emitir un elemento a la vez`, manteniendo un control exacto sobre cuándo se produce, se completa o se
lanza un error en el flujo.

💡 A diferencia de `Flux.create()`, que permite emisiones desde múltiples hilos y en cualquier momento,
`Flux.generate()` se ejecuta `desde un solo hilo y no es thread-safe`. Su diseño es deliberadamente imperativo
y controlado paso a paso.

### 🧭 Comportamiento y restricciones clave

- 🔁 Invoca la expresión lambda una vez por cada solicitud de elemento (`request`) del suscriptor.
- 🎯 Solo se permite `una llamada` a `.next()` por iteración. Si se emite más de un elemento en la misma invocación,
  Reactor lanza un error.
- 🧱 El flujo `se detiene` cuando se llama a:
    - `complete()` → marca el flujo como finalizado.
    - `error(Throwable)` → marca el flujo como fallido.
- 🚫 Si se intenta emitir múltiples valores o continuar tras completarse, se lanza:
    ````bash
    error: More than one call to onNext
    ````

### 💥 Ejemplo: Intento de múltiples emisiones
...
public class Lec06FluxGenerate {
    public static void main(String[] args) {
        Flux.generate(synchronousSink -> {
            synchronousSink.next(1);    // ✅ OK: primera emisión
            synchronousSink.next(2);    // ❌ ERROR: no está permitido
            synchronousSink.complete(); // no se ejecuta
        }).subscribe(Util.subscriber());
    }
}
````
...
Solo usamos una única vez el `.next(...)`.

````java
public class Lec06FluxGenerate {
    public static void main(String[] args) {
        Flux.generate(synchronousSink -> {
            synchronousSink.next(1);        // ✅ solo una emisión
            synchronousSink.complete();     // ✅ flujo finalizado
        }).subscribe(Util.subscriber());
    }
}
````
...
> `Flux.generate()` es ideal para flujos controlados, paso a paso, donde la generación depende del estado interno o de
> condiciones secuenciales. Si necesitas emitir datos desde múltiples hilos o fuentes asíncronas, usa `Flux.create()`
> en su lugar.
...
## 🔒 Concurrencia y seguridad de hilos con `Flux.generate()`

A diferencia de `Flux.create()`, el operador `Flux.generate()` `no es seguro para subprocesos` (`not thread-safe`).
Esto significa que `solo debe usarse desde un único hilo`, ya que `generate()` mantiene estado interno entre cada
invocación, lo que puede corromperse si varios hilos intentan emitir al mismo tiempo.

`Flux.generate()` está diseñado para escenarios `secuenciales y deterministas`, donde cada emisión depende del paso
anterior. Por eso, su ejecución se hace de manera síncrona y controlada por el backpressure del suscriptor.

### ✅ Ejemplo: uso correcto desde un solo hilo
...
````bash
18:22:42.195 INFO  [           main] .Lec08FluxGenerateSingleThread : Emitiendo valor: 1
18:22:42.201 INFO  [           main] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 1
18:22:42.201 INFO  [           main] .Lec08FluxGenerateSingleThread : Emitiendo valor: 2
18:22:42.201 INFO  [           main] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 2
18:22:42.201 INFO  [           main] .Lec08FluxGenerateSingleThread : Emitiendo valor: 3
18:22:42.202 INFO  [           main] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 3
18:22:42.202 INFO  [           main] .Lec08FluxGenerateSingleThread : Emitiendo valor: 4
18:22:42.202 INFO  [           main] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 4
18:22:42.202 INFO  [           main] .Lec08FluxGenerateSingleThread : Emitiendo valor: 5
18:22:42.202 INFO  [           main] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 5
18:22:42.206 INFO  [           main] d.m.a.common.DefaultSubscriber :  ¡completado!
````

💡 Explicación:

- Cada invocación del generador mantiene su propio estado (`state`), en este caso un contador.
- Reactor llama al generador una vez por elemento solicitado.
- Todo el proceso ocurre en el mismo hilo —el del suscriptor— garantizando la consistencia.

### ❌ Ejemplo: uso incorrecto con múltiples hilos
...
public class Lec08FluxGenerateSingleThread {
    ...
    private static void incorrectGenerate() {
        // Estado compartido entre hilos (aquí está el problema)
...
````bash
23:27:46.627 INFO  [         Hilo-0] .Lec08FluxGenerateSingleThread : Emitiendo valor : 1
23:27:46.627 INFO  [         Hilo-2] .Lec08FluxGenerateSingleThread : Emitiendo valor : 3
23:27:46.627 INFO  [         Hilo-1] .Lec08FluxGenerateSingleThread : Emitiendo valor : 2
23:27:46.632 INFO  [         Hilo-0] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 1
23:27:46.632 INFO  [         Hilo-2] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 3
23:27:46.632 INFO  [         Hilo-0] .Lec08FluxGenerateSingleThread : Emitiendo valor : 5
23:27:46.632 INFO  [         Hilo-1] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 2
23:27:46.632 INFO  [         Hilo-0] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 5
23:27:46.632 INFO  [         Hilo-2] .Lec08FluxGenerateSingleThread : Emitiendo valor : 4
23:27:46.632 INFO  [         Hilo-1] .Lec08FluxGenerateSingleThread : Emitiendo valor : 6
23:27:46.632 INFO  [         Hilo-2] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 4
23:27:46.632 INFO  [         Hilo-1] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 6
23:27:46.632 INFO  [         Hilo-2] .Lec08FluxGenerateSingleThread : Emitiendo valor : 7
23:27:46.632 INFO  [         Hilo-2] d.m.a.common.DefaultSubscriber :  recibido: Valor -> 7
23:27:46.636 INFO  [         Hilo-1] d.m.a.common.DefaultSubscriber :  ¡completado!
23:27:46.636 INFO  [         Hilo-0] d.m.a.common.DefaultSubscriber :  ¡completado!
23:27:46.636 INFO  [         Hilo-2] d.m.a.common.DefaultSubscriber :  ¡completado!
````

### 💥 Qué está pasando técnicamente:

**Diferencia clave con el ejemplo correcto:**

| Aspecto       | ✅ Versión correcta                                                        | ❌ Versión incorrecta                                                       |
|---------------|---------------------------------------------------------------------------|----------------------------------------------------------------------------|
| **Estado**    | `() -> 1` - Cada suscripción tiene estado independiente dentro de Reactor | `sharedState` - Todos los hilos comparten el mismo `AtomicInteger` externo |
| **Hilos**     | Un solo hilo (main) ejecuta todo secuencialmente                          | Tres hilos diferentes compiten por el mismo contador                       |
| **Secuencia** | Ordenada y predecible: 1→2→3→4→5                                          | Caótica y distribuida entre hilos                                          |

**Problemas observados en la salida:**

1. **Valores no secuenciales por hilo**:
    - **Hilo-0** recibe: 1, 5 (se salta 2, 3, 4)
    - **Hilo-1** recibe: 2, 6 (se salta 3, 4, 5)
    - **Hilo-2** recibe: 3, 4, 7 (se salta 5, 6)

   Cada hilo ve una secuencia "saltada" porque los otros hilos están consumiendo los valores intermedios.

2. **Condición de terminación inconsistente**:
    - **Hilo-0** termina cuando lee `state = 5` ✓
    - **Hilo-1** termina cuando lee `state = 6` (debería terminar en 5)
    - **Hilo-2** termina cuando lee `state = 7` (debería terminar en 5)

   Solo el primer hilo que alcanza `>= 5` termina "correctamente", los demás continúan más allá.

3. **Race conditions en el contador compartido**:
    - Los tres hilos ejecutan `getAndIncrement()` simultáneamente
    - El contador avanza de forma impredecible: 1→2→3→4→5→6→7
    - Los valores se distribuyen aleatoriamente entre los hilos según quién llegue primero

4. **Pérdida de la semántica de `generate()`**:
    - `Flux.generate()` está diseñado para que cada emisión sea **consecuencia de la anterior**
    - Con múltiples hilos, esta cadena de dependencia se rompe
    - Cada hilo no ve el "historial" completo de emisiones, solo fragmentos aleatorios

💡 **Explicación conceptual**

`Flux.generate()` fue diseñado para generaciones síncronas y deterministas, controladas por el flujo de demanda
(`request`) del suscriptor.

**No soporta la concurrencia porque:**

- ❌ **No usa mecanismos de sincronización internos** - Reactor asume que cada suscripción opera en un contexto de un
  solo hilo.
- ❌ **No está preparado para acceso concurrente al estado** - La función generadora espera ver una progresión lineal del
  estado.
- ❌ **La secuencia generada asume que solo un hilo avanza el estado en cada paso** - Cada invocación debería ver el
  resultado de la invocación anterior, no valores aleatorios.
- ❌ **Cada suscripción debería ser independiente** - Compartir estado entre suscripciones rompe el modelo reactivo de
  Reactor.

### 🎯 Conclusión:

Aunque usamos `AtomicInteger` para evitar corrupción de memoria, **el flujo lógico se destruye completamente**. Este
ejemplo demuestra por qué `Flux.generate()` debe ejecutarse en un contexto de un solo hilo y por qué no debes compartir
estado mutable entre múltiples suscripciones.

**Para emisión desde múltiples hilos, usa `Flux.create()` en su lugar**, que sí está diseñado para manejar concurrencia
de forma segura.
...
## 🧭 Resumen: Flux.create() vs Flux.generate()

| Aspecto                                 | `Flux.create()`                                                                             | `Flux.generate()`                                                                                                                     |
|-----------------------------------------|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| **Firma**                               | Acepta un `Consumer<FluxSink<T>>`                                                           | Acepta un `Consumer<SynchronousSink<T>>`                                                                                              |
| **Invocación del consumidor**           | Se invoca **una sola vez**, cuando se suscribe el flujo.                                    | Se invoca **una y otra vez**, controlado por la **demanda del suscriptor** (*backpressure*).                                          |
| **Cantidad de emisiones por iteración** | Puede emitir **0..N elementos** de forma inmediata, sin depender de la demanda.             | Solo puede emitir **exactamente un elemento** por iteración (`.next()` una sola vez).                                                 |
| **Control del flujo (backpressure)**    | El desarrollador **no necesita manejarlo**; Reactor se encarga internamente.                | **Respeta estrictamente** la demanda del suscriptor (solo emite cuando hay `request`).                                                |
| **Uso típico**                          | Ideal para integrar **código imperativo o APIs asíncronas** (callbacks, listeners, hilos).  | Ideal para **generar datos secuenciales, síncronos y deterministas** (contadores, simulaciones, etc.).                                |
| **Seguridad de hilos (Thread safety)**  | ✅ **Thread-safe** – permite emitir desde múltiples hilos simultáneamente.                   | ⚠️ **No thread-safe** – debe ejecutarse desde **un solo hilo**; emitir desde varios puede producir valores duplicados o desordenados. |
| **Naturaleza de emisión**               | **Asíncrona** – varios hilos pueden llamar a `next()`, y Reactor gestiona la concurrencia.  | **Síncrona** – la generación ocurre paso a paso, dentro del mismo hilo.                                                               |
| **Ejemplo práctico**                    | Integrar un flujo de datos que proviene de múltiples hilos (por ejemplo, sensores o colas). | Generar una secuencia numérica o flujo controlado (`Flux.generate(() -> 1, (s, sink) -> ...)`).                                       |

### 💡 Conclusión rápida:

- Usa `Flux.create()` → cuando necesites emisiones concurrentes o múltiples fuentes.
- Usa `Flux.generate()` → cuando necesites un flujo controlado, paso a paso y determinista.