...
#### 📝 Nota sobre la Implementación del Mapa

> La implementación `Collections.synchronizedMap(new HashMap<>())` utilizada en este código garantiza la seguridad de
> hilos, pero utiliza un único bloqueo para todo el mapa. Como alternativa más moderna y eficiente, especialmente en
> entornos de alta concurrencia, se podría considerar:
>
> - `ConcurrentHashMap`: Ofrece un mejor rendimiento al permitir que múltiples hilos operen simultáneamente en
    diferentes partes del mapa, minimizando la contención de bloqueos.
> - `Contadores Atómicos` (AtomicInteger dentro del mapa): Esta sería la forma ideal para manejar contadores, ya que el
    decremento (`decrementAndGet()`) se vuelve una operación atómica intrínseca, eliminando la necesidad de sincronizar
    todo el método canAllow.
...
public class RateLimiter {
    ...
    // Se agrega el modificador 'synchronized' para garantizar la atomicidad de la operación.
    ...
    private static void refresh() {
        Flux.interval(Duration.ofSeconds(5))        // El primer valor lo emite después de 5 segundos, por eso nos apoyamos en el startWith(...)
...
#### ⚠️ Justificación del Modificador synchronized

Aunque el mapa `categoryAttemps` ya es seguro para hilos gracias a `Collections.synchronizedMap()`, lo cual asegura
que operaciones individuales como `get()` y `put()` sean seguras, la lógica del limitador de tasas dentro de
`canAllow(..)` `NO es atómica`:

1. `Lectura`: `Integer attemps = categoryAttemps.getOrDefault(category, 0);`
2. `Verificación`: `if (attemps > 0)`
3. `Escritura/Actualización`: `categoryAttemps.put(category, attemps - 1);`

> 🚨 El método `canAllow(...) NO es atómico` y tiene una condición de carrera (Race Condition).

#### 🧩 El Problema de la Condición de Carrera (Race Condition)

La secuencia de `Lectura`, `Verificación` y `Escritura` no está protegida como una unidad. Si dos hilos
(`Hilo A` y `Hilo B`) llaman a `canAllow(...)` al mismo tiempo y el contador es, por ejemplo, `1`, pueden leer el
valor original antes de que el otro hilo lo actualice, lo que lleva a un decremento incorrecto.

`Resultado Incorrecto`: El sistema podría permitir una llamada extra, violando el límite de tasas.

Observemos el método `canAllow(...)` (sin el `synchronized`):

````java
private static boolean canAllow(Category category) {
    Integer attemps = categoryAttemps.getOrDefault(category.getName(), 0); // (1) Lectura
    if (attemps > 0) {
        categoryAttemps.put(category.getName(), --attemps); // (2) Escritura
        return true;
    }
    return false;
}
...
A pesar de usar `Collections.synchronizedMap(new HashMap<>())`, el cual asegura la atomicidad de las operaciones
individuales, la secuencia de operaciones `(1) Leer` y `(2) Escribir/Actualizar` `NO es atómica` como un todo.

Escenario de Falla

- `Hilo A` lee: attemps para "STANDARD" es 2.
- `Hilo B` lee casi simultáneamente: attemps para "STANDARD" es 2.
- `Hilo A` verifica `attemps > 0` (True), calcula nuevo valor (1), y escribe 1 en el mapa.
- `Hilo B` verifica `attemps > 0` (True), calcula nuevo valor (1), y escribe 1 en el mapa
  (sobrescribiendo el valor de A).

`Resultado`: Se permitieron 2 llamadas, pero el contador terminó en 1, no en 0 como debería. El sistema permitió una
llamada extra y no limitó correctamente la tasa.

#### 🧩 Solución Aplicada (Uso de `synchronized`)

Para resolver esta `Condición de Carrera`, se debe asegurar que el bloque completo de operaciones se ejecute como
una única `operación atómica`.

- Al añadir el modificador `synchronized` a la firma del método `static`:

````bash
private static synchronized boolean canAllow(String category)
````

Se obliga a que solo un hilo a la vez pueda ejecutar el método `canAllow(...)`. Esto asegura que el hilo que ingresa
primero complete la operación de lectura y escritura antes de que cualquier otro hilo pueda acceder al contador,
garantizando que el límite se aplique de forma estricta.
